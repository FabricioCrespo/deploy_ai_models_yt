...
def load_lightweight_model():
    ...
    try:
        model = joblib.load('./model/lightweight_recommender.pkl')
        user_encoder = joblib.load('./data_processed/user_encoder.pkl')
...
def recommend_for_user(user_id, model, user_encoder, top_n=10):
    ...
    movies = pd.read_csv('./data_processed/movies_processed.csv', 
                        usecols=['movieId', 'year'])
    
    # Cargar ratings del usuario (eficiente)
    user_ratings = pd.read_csv('./data_processed/ratings_processed.csv',
    ...
    # Obtener títulos
    movie_titles = pd.read_csv('movie.csv', usecols=['movieId', 'title'])

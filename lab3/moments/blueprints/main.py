...
def generate_caption_and_tags(image_path):
    """Generates a caption and extracts relevant tags using NLP."""
    ...
    # Extract relevant tags using NLP (nouns + adjectives)
    doc = nlp(caption)
    tags = {token.text.capitalize() for token in doc if token.pos_ in ["NOUN", "ADJ"]}

    # Limit to 3 most relevant tags
    return caption, list(tags)[:3]
...
def upload():
    if request.method == 'POST':
        ...
        image_path = str(current_app.config['MOMENTS_UPLOAD_PATH'] / filename)  # Convert to string
        f.save(image_path)
        ...
        # ✅ Generate caption and accurate tags
        caption, tag_names = generate_caption_and_tags(image_path)

        # ✅ Get or create tags
        tags = []
        for tag_name in tag_names:
            tag = db.session.execute(select(Tag).filter_by(name=tag_name)).scalar_one_or_none()
            if not tag:
                tag = Tag(name=tag_name)
                db.session.add(tag)
            tags.append(tag)

        db.session.flush()  # Ensure tags have IDs before assigning

        # ✅ Save photo with generated caption and tags
        photo = Photo(
            filename=filename, 
            filename_s=filename_s, 
            filename_m=filename_m, 
            author=current_user._get_current_object(),
            description=caption,  # Use generated caption
            tags=tags  # Assign extracted tags
        )
